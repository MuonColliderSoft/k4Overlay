...
DECLARE_COMPONENT(OverlayInput)

template<typename T> inline void OverlayInput::maybeRead(
        std::string_view collName) const
{
    if (m_overlayDataSvc->readCollection < T
            > (std::string(collName)).isFailure())
    {
        error() << "Failed to register collection " << collName << endmsg;
    }
}

void OverlayInput::fillReaders()
{
...
OverlayInput::OverlayInput(const std::string &name, ISvcLocator *svcLoc) :
        Consumer(name, svcLoc)
{
    // check whether we have the PodioEvtSvc active
    m_overlayDataSvc = dynamic_cast<OverlayDataSvc*>(evtSvc().get());
    if (!m_overlayDataSvc)
    {
...
StatusCode OverlayInput::initialize()
{
...
void OverlayInput::operator()() const
{
    if (m_overlayDataSvc->getEventFrame().get(edm4hep::EventHeaderName))
    {
        m_readers[edm4hep::EventHeaderCollection::typeName](
                edm4hep::EventHeaderName);
    }
    else
    {
        info() << "No EventHeader collection found in the event. Not reading it"
                << endmsg;
    }
    ...
    const auto &collsToRead = [&]()
    {
        ...
        {
            return m_overlayDataSvc->getEventFrame().getAvailableCollections();
        }
        else
        {
    ...
    for (const auto &collName : collsToRead)
    {
        ...
        if (!m_overlayDataSvc->getEventFrame().get(collName))
        {
            warning() << "Collection " << collName
                    << " is not available from file." << endmsg;
            continue;
        }
        auto type = m_overlayDataSvc->getCollectionType(collName);
